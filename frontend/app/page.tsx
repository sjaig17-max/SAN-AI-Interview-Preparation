...
export default function AuthPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Fallback email if they haven't typed anything
    const email = formData.email.trim() || "sjaig17@gmail.com";
    const password = "password123";
    
    // Derive a name if not entered
    const namePart = email.split("@")[0];
    const derivedName = formData.full_name.trim() || (namePart.charAt(0).toUpperCase() + namePart.slice(1));
    ...
    try {
      ...
      // 2. If user does not exist (401), auto-register and then log in
      if (err.response?.status === 401 || err.response?.data?.detail === "Incorrect email or password.") {
        try {
          ...
          setError(regErr.response?.data?.detail || "Google Registration failed.");
        }
      } else {
            ...
            {isLogin ? (
              ...
              >
                ...
                <button
                  ...
                  disabled={loadingAction}
                  className="w-full py-3 px-4 bg-white/5 hover:bg-white/10 active:scale-95 text-white font-semibold rounded-xl border border-white/10 transition-all flex items-center justify-center gap-3 text-sm disabled:opacity-50"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24">
                    <path
                      fill="#EA4335"
                      d="M5.266 9.765A7.077 7.077 0 0 1 12 4.909c1.69 0 3.218.6 4.418 1.582l3.51-3.51C17.745 1.055 14.99 0 12 0 7.354 0 3.307 2.68 1.255 6.602l4.01 3.163z"
                    />
                    <path
                      fill="#4285F4"
                      d="M23.49 12.275c0-.825-.075-1.62-.215-2.385H12v4.51h6.443a5.503 5.503 0 0 1-2.385 3.61l3.71 2.875c2.17-2 3.422-4.945 3.422-8.61z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M5.266 14.235L1.255 17.398A11.967 11.967 0 0 0 12 24c2.99 0 5.745-1.055 7.91-2.875l-3.71-2.875a7.077 7.077 0 0 1-10.934-4.015z"
                    />
                    <path
                      fill="#34A853"
                      d="M12 4.909c1.93 0 3.67.665 5.03 1.964l3.51-3.51C17.745 1.055 14.99 0 12 0c-4.646 0-8.693 2.68-10.745 6.602l4.01 3.163a7.077 7.077 0 0 1 6.735-4.856z"
                    />
                  </svg>
                  Google
                </button>
